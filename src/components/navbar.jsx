...
//         ...router,
//         checked: allChecked,
//         computers: router.computers.map((pc) => ({
//           ...pc,
//           checked: allChecked,
//         })),
//       }));
...
//           return {
//             ...router,
//             computers: router.computers.map((pc, j) => {
//               if (j === pcIndex) {
//                 return {
//                   ...pc,
//                   checked: !pc.checked,
...
//   const handleScan = () => {
//     const updatedRouters = routers.map((router) => {
//       const isRouterChecked = router.checked || router.computers.some((pc) => pc.checked);
...
//     if (routers.length === 0) {
//       const initializedRouters = initialRouters.map((router) => ({
//         ...router,
//         pcs: router.pcs.map((pc) => ({
//           ...pc,
//           router_id: router.id,
...
//         router.pcs.forEach((pc) => (pc.checked = allChecked));
//       });
//     } else if (type === "router") {
//       updatedRouters[index].checked = !updatedRouters[index].checked;
//       updatedRouters[index].pcs.forEach(
//         (pc) => (pc.checked = updatedRouters[index].checked)
//       );
//     } else if (type === "pc") {
//       updatedRouters[index].pcs[pcIndex].checked = !updatedRouters[index].pcs[pcIndex].checked;
//       // Проверяем, все ли PC в данном роутере выбраны, чтобы установить checked для роутера
//       updatedRouters[index].checked = updatedRouters[index].pcs.every((pc) => pc.checked);
//     }
...
//   const handleScan = () => {
//     const updatedRouters = routers.map((router) => {
//       // Проверяем, выбран ли роутер или хотя бы один из его ПК
//       const isRouterChecked = router.checked || router.pcs.some((pc) => pc.checked);
  
//       return {
//         ...router,
//         checked: isRouterChecked,
//         opacity: isRouterChecked ? 1 : 0.2,
//         pcs: router.pcs.map((pc) => ({
//           ...pc,
//           opacity: pc.checked ? 1 : 0.2,
...
//     // Обновляем состояние и передаем данные в родительский компонент
//     setRouters(updatedRouters);
//     onScan(updatedRouters);
//   };
...
//           </div>
//           <div className="filter-left-nav-bar">
//             <label style={{ color: "#FFFFFF", marginLeft: "30px", marginTop: "15px", marginBottom: "10px" }}>
//               <input
//                 type="checkbox"
//                 checked={routers.every(
//                   (router) => router.checked && router.pcs.every((pc) => pc.checked)
...
//               <div key={router.id}>
//                 <label style={{ color: "#FFFFFF", marginLeft: "15px", marginBottom: "5px" }}>
//                   <input
//                     type="checkbox"
...
//                 {router.pcs.map((pc, pcIndex) => (
//                   <div key={pc.id} style={{ marginLeft: "20px" }}>
//                     <label style={{ color: "#FFFFFF", marginLeft: "15px", marginBottom: "10px" }}>
//                       <input
//                         type="checkbox"
